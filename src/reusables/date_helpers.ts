...
export const js_to_utc_date_string = (js_date: Date) =>
    js_date.toISOString().slice(0, 10)

export const get_days_between_dates = (
    start_date: Date,
    end_date: Date
): number => {
...
export const format_mysql_date = (date: string) =>
    format_date(mysql_to_js_date(date))

export const format_time = (date: Date) => format(date, 'h:mm a')

export const format_mysql_time = (date: string) =>
    format_time(mysql_to_js_date(date))

export const format_date_time = (date: Date) =>
    format(date, 'dd MMM yyyy, h:mm a')

export const format_mysql_date_time = (date: string) =>
    format_date_time(mysql_to_js_date(date))